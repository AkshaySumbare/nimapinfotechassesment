...
export const UpcommingMovies = () => {
  ...
  const handleSearch = async (event) => {
    ...
    try {
      const res = await fetch(
        `https://api.themoviedb.org/3/search/movie?api_key=c45a857c193f6302f2b5061c3b85e743&language=en-US&query=${searchQuery}`
      );
  ...
  const getPaginatedUser = async () => {
    try {
      const res = await fetch(
        `https://api.themoviedb.org/3/movie/upcoming?api_key=c45a857c193f6302f2b5061c3b85e743&language=en-US&page=${currentPage.current}`
      );
      const data = await res.json();
